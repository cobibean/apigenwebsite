...
export default function BrandDetails({ brand, preview }: BrandDetailsProps) {
  return (
    <section className="relative border-t border-muted bg-background">
      ...
      <div className={`relative mx-auto flex w-full max-w-6xl flex-col justify-center gap-10 px-6 md:gap-12 lg:grid lg:grid-cols-[minmax(0,_1.15fr)_minmax(0,_0.85fr)] lg:items-center lg:gap-16 ${SPACING.section}`}>
        <div className="space-y-6 md:space-y-8">
          <span className="sr-only">{brand.name}</span>
          <h2 className="max-w-[18ch] text-balance font-semibold tracking-tight text-primary text-[clamp(2.0rem,3.2vw,3.0rem)] lg:text-[clamp(2.2rem,3.0vw,3.2rem)] font-sans">
            {brand.heading}
          </h2>
      ...
      <div
        ...
        aria-hidden="true"
      />
      <div className={`mx-auto flex w-full max-w-6xl flex-col px-6 py-14 md:py-16 lg:grid lg:grid-cols-[minmax(0,_1.2fr)_minmax(0,_1fr)] ${SPACING.gridGap}`}>
