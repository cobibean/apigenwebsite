...
| --- | --- | --- | --- | --- |
| `NEXT_PUBLIC_SUPABASE_URL` | Both | **Yes** | `https://xxxxx.supabase.co` | Your Supabase project URL. Required for all database operations, CMS content, carousels, and admin authentication. Found in Supabase Dashboard → Settings → API. |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Both | **Yes** | *(none, keep secret)* | Supabase anonymous/public key for client-side operations. Required for content fetching and public queries. Found in Supabase Dashboard → Settings → API. |
| `SUPABASE_SERVICE_ROLE_KEY` | Server | Yes | *(none, keep secret)* | Supabase service role key for admin operations. Used by seed scripts and `/api/admin/images/unused/delete`. **Never expose to client.** Found in Supabase Dashboard → Settings → API. |
...
The entire CMS, carousel system, and admin authentication depend on Supabase. The following environment variables are **critical** for the application to function:

- `NEXT_PUBLIC_SUPABASE_URL` – Your Supabase project URL (e.g., `https://ckzfibtiqllgrjhfvrjd.supabase.co`). Found in Supabase Dashboard → Settings → API → Project URL.
- `NEXT_PUBLIC_SUPABASE_ANON_KEY` – The anonymous/public API key for client-side operations. This is safe to expose in the browser but should still be kept in environment variables. Found in Supabase Dashboard → Settings → API → anon public.
- `SUPABASE_SERVICE_ROLE_KEY` – The service role key with full database access. **Server-only, never expose to client.** Used by seed scripts and admin API routes. Found in Supabase Dashboard → Settings → API → service_role secret.

### Where these are used:

| Variable | Used By |
| --- | --- |
| `NEXT_PUBLIC_SUPABASE_URL` | `src/lib/supabase/*.ts`, `src/middleware.ts`, all seed scripts |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | `src/lib/supabase/*.ts`, `src/middleware.ts` |
| `SUPABASE_SERVICE_ROLE_KEY` | `scripts/seed-*.ts`, `/api/admin/images/unused/delete` |

If these variables are missing, the application will gracefully degrade:
- Content sections fall back to static data from `src/data/`
- Carousels fall back to static gallery images
- Admin routes will show "Missing Supabase env vars" messages
